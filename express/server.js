...
app.get('/:url', function(reg, res){
    var url = reg.params.url;
    //根据url取得相关衍生版本
    var json = require('./data/?.json'.replace('?', btoa(url)));
    res.send(json ? {code:1, json:json, msg:'目前显示的是衍生版本'} : {code:0, msg:'还没有衍生版本，你可以创建一个衍生版本哦'});
});

app.post('/:url', function(reg, res){
    var url = reg.params.url;
    var content = JSON.stringify(req.body);
    console.log(content);
    fs.appendFile('./data/?.json'.replace('?', btoa(url)), content, function(err){
        console.log(err);
    });
